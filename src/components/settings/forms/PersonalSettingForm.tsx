...
interface PersonalSettingFormProps {
  formData: {
    setting_key: string;
    setting_value: {
      value?: string;
...
const personalSettingTypes = [
  { value: 'title', label: 'Title (Mr, Mrs, Ms, etc.)' },
...
export function PersonalSettingForm({ 
  formData, 
  onFormDataChange, 
  onSave, 
  onCancel, 
  isEditing 
}: PersonalSettingFormProps) {
  const updatePersonalValue = (field: string, value: string) => {
    onFormDataChange({
      ...formData,
      setting_value: {
    ...
    <Card className="border-dashed border-primary/20 bg-gradient-to-br from-background to-muted/20">
      <CardContent className="p-6 space-y-6">
        <div className="flex items-center gap-3 pb-4 border-b border-border/50">
          ...
          <div>
            <h3 className="font-semibold text-lg">Personal Information Option</h3>
            <p className="text-sm text-muted-foreground">Add options for personal information fields</p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => {
                  updatePersonalValue('value', e.target.value);
                  onFormDataChange({
                    ...formData,
                    setting_key: `${formData.setting_type || 'option'}_${e.target.value}`
                  });
                }}
                placeholder="e.g., Mr, British, Male, etc."
                className="mt-2"
              />
            </div>
          </div>
